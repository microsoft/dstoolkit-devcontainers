...
└── src
    ├── common
    │   └── requirements.txt       # includes python package list for common module. this common module is accessible from all modules under src so put functions such as data preprocessing you want to use across the projects here. installed in all Dockerfile under src. python tools for src goes to here too
...
You got two places to put python scripts/modules in this template. `notebooks` directory is for interactive python scripts/jupyter notebooks that you want to run cell by cell interactively. For example, EDA, one-off visualization codes, new model approaches you are not certain yet if you want to maintain overtime typically go to this directory. `src` directory is for python scripts/modules that you want to run in command line, you want to maintain and reuse overtime. `src` directory is also where you put your unit tests. Given the nature of each directory's responsibility, there are different quality governance. One big difference is that pre-commit hooks and CI pipelines run flake8 over `src` but not over `notebooks` (isort and bandit still run for both). For scripts in `notebooks`, we recommend you to use [interactive python scripts](https://code.visualstudio.com/docs/python/jupyter-support-py#_convert-jupyter-notebooks-to-python-code-file) where you can have Jupyter-like code cells within `.py` over jupyter notebooks `.ipynb`. That format gives you the following benefits:

- Comes with full benefits of python extension in VSCode such as code completion, linting, auto formatting, debugging etc
- pre-commit hooks and CI pipelines work support python scripts
- Python file format is easier to review during pull request review
...
1. Copy `.env.example` and rename it to `.env`. This is where you store your credentials etc. `.env` is automatically loaded into dev container as environment variables. When you add new environment variables `.env`, update `.env.example` as well to share that with others but don't include any credentials there. `.env` is gitignored so your credentials in that file won't be accidentally committed. If you forget this step, you will get an error when you try to build the container so make sure you have `.env` at root of this directory before you move on to the next step.
